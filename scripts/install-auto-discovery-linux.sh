...
printf '%s\n' \
  ...
  "Timer: active (1 hour, randomized delay up to 60 seconds)" \
  "Policy: auto-completable public query >= 0.01; analysis/software/content/remote >= 1; quant risk >= 10 USDT" \
  "Behavior: rank by budget/category-floor value score; start at most one new negotiation per hourly scan; never call apply in the discovery loop" \
  "Backup: $backup"
